...
func HandleLandingIndex(kit *kit.Kit) error {
	...
	var videoConfig types.LandingVideo
	// Try to get the first (and only) video
	if err := db.Get().First(&videoConfig).Error; err != nil {
	...
	return RenderWithLayout(kit, landing.Index(successMessages, failMessages, caroucelImages, config, videoConfig))
}
...
func Gallerie(kit *kit.Kit) error {
	var images []types.ImageWithComments
	return RenderWithLayout(kit, landing.Gallerie(images))
}
