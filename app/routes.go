...
func InitializeRoutes(router *chi.Mux) {
	...
	router.Group(func(app chi.Router) {
		...
		app.Get("/photo+view", kit.Handler(handlers.Gallerie))
		...
		app.Post("/payments/webhook", kit.Handler(func(kit *kit.Kit) error {
			...
			ref := kit.Request.FormValue("payment_ref")
			log.Println("Request Parms:", ref)
			...
			return kit.Text(http.StatusOK, string(body))
		}))
	...
	router.Group(func(app chi.Router) {
		...
		app.Get("/admin/video-landing", kit.Handler(video.VideoLandingConfig))
		app.Get("/payments/status", kit.Handler(handlers.HandlePaymentStatus))
		app.Get("/gallery/upload", kit.Handler(handlers.GalleryUploadForm))
		app.Get("/gallery/titels", kit.Handler(handlers.HandleGalleryTitles))

		app.Post("/gallery/upload", kit.Handler(handlers.HandleGalleryUpload))
		app.Post("/admin/landing/video/save", kit.Handler(conf.SaveLandingVideoHandler))
		...
		app.Post("/admin/carousel/toggle", kit.Handler(conf.ToggleCarouselHandler))
		app.Post("/user/konnect/payment/Init", kit.Handler(handlers.Konnect))
