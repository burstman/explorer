...
func (s *KonnectService) CreateKonnectPayment(userid int, payment *types.KonnectPaymentResponse) error {
...
func (s *KonnectService) doPostRequest(req types.InitPaymentRequest, path string) ([]byte, error) {
	...
	log.Println("HTTP REQ: ", s.BaseURL+path)
	if err != nil {
		return nil, err
	}
...
func (s *KonnectService) InitPayment(userid int, req types.InitPaymentRequest) (types.PaymentResponse, error) {

	data, err := s.doPostRequest(req, "/payments/init-payment")
	if err != nil {
		log.Println("err in InitPayment:", err)
		return nil, err
	}
	...
	if err := s.CreateKonnectPayment(userid, resp); err != nil {
		return nil, err
	}
